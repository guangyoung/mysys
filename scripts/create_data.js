      ...
      if(asset_portfolio_yahoo.length < 30) {
        ...
        //cek periode data yg tercover oleh semua stocks
          ...
          while (startDate <= endDate) {
            ...
            as_arr.push(dtt);          
            for (y=0; y<3; y++) { //CEK BAGAIMANA PROSES INI BISA CEPAT....PENTIIIING !!!!!!
              for (i=y*10; i<((y+1)*10); y++) { //CEK BAGAIMANA PROSES INI BISA CEPAT....PENTIIIING !!!!!!
                let idx = asset_portfolio_yahoo[i].data.date.indexOf(dtt);
                if(idx == -1) {//jika idx tidak ditemukan
                  as_arr.push(port_data[port_data.length-1][i+1]); //masukkan harga sebelumnya
                } else {
                  as_arr.push(asset_portfolio_yahoo[i].data.price[idx]); //jika idx ketemu masukkan harga berdasarkan idx
                }
              }
            }
